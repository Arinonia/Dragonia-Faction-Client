...
public class GuiScreen extends Gui
{
    ...
    public void drawDefaultBackground(int x, int y)
    {
        ...
        {
            drawBack((int)getYaw(x), (int)getPitch(y));

        }
    }

    private int getDistanceTo(int z, boolean e){
        if (e){
            return Math.abs(this.width / 2 - z);
        }
        return Math.abs(this.height / 2 - z);
    }
    ...
    public void drawBack(int x, int y){
        ...
        mc.getTextureManager().bindTexture(new ResourceLocation("textures/gui/dragonia/wallpaper.png"));
        GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
        this.drawTextureWithOptionalSize(-1, -1, x, y, width + 5, height + 5, width+5, height+5);
    }

    public float getYaw(int x){
        if (x >= this.width / 2){
            return -getDistanceTo(x, true) / 35;
        }else{
            return getDistanceTo(x, true) / 35;

        }
    }
    protected float getPitch(int x){
        if (x >= this.height / 2){
            return -getDistanceTo(x, false) / 35;
        }else{
            return getDistanceTo(x, false) / 35;
